...
| Recall interval | Proxy scaled from the AUC interval | Vehicle-cluster bootstrap of event-level recall at every alert rate (`stats.event_recall_ci`); the operating point and ROI consume it and record `recall_ci_basis` | The ROI's dominant input had a made-up interval; now it is measured (fixture: 0.197, 0.152 to 0.238) |
| Tuning headroom | Fixed configuration, unexamined | Fixed 3-configuration LightGBM grid on the sufficient set, paired delta vs default, `loose_lower_bound`; verdict keeps the default | Says how loose the lower bound is without tuning studies to their own noise (fixture: +0.003, noise) |
| Seed stability | Determinism only | Sufficient-set AUC under 3 fold seeds; `seed_sensitive` flag (policy 1.4) at the ablation tolerance | Same seed same answer is not the same as a different seed same answer; on 600 vehicles the spread is 0.012 and the flag trips |
| Policy sensitivity | Thresholds fixed, margins invisible | `verdict.sensitivity`: margin of every gate/flag in declared step units (`sensitivity_steps`), binding gate, decision with all thresholds one step against / in favour, `robust` | Makes fixed thresholds reviewable; the fixture brake verdict sits at exactly the coverage gate (+0.0 steps) and is not robust |
| Policy inputs | `learning_curve` computed but not in `POLICY_INPUTS` | Added, with a test that every `ev.get` in `decide` is fed by the runner | `data_still_improving` had never fired in a real study |
| Access | None in the app | Optional `MDE_API_KEY` bearer auth (health open), constant-time compare; dashboard proxy adds the header server-side; `MDE_CORS_ORIGINS`; `MDE_LLM_PROVIDER=none` | Belt under the network control described in DEPLOY.md |
| Labels | Implicit in the synthetic generator | `docs/LABELS.md`: per target event the proposed source of truth, event-date rule, known-at, exclusions, leakage to watch, sanity checks | The first thing a real-data run needs the data owners to confirm |
